        ...
        <div class="row">
            <div class="col-xl-11 col-lg-12 mx-auto">
                ...
                <form id="laporanForm" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="tipe_laporan" value="VMTS">
                        ...
                        <div style="padding: 1.5rem;">
                            <div class="row">
                                <div class="col-md-6 mb-4">
                                    ...
                                    <select class="form-select" name="periode_VMTS" id="periode_VMTS" required>
                    ...
                    <div class="monitoring-card mb-4" id="ai-prompt-section" style="overflow: hidden;">
                        ...
                        <div style="padding: 1.5rem;">
                            <!-- Laporan VMTS AI Assistant Interface -->
                            <style>
                            ...
                            <div class="ai-chat-wrapper">
                                <div class="ai-chat-header">
                                    ...
                                    <div>
                                        <h6>AI Assistant - Laporan VMTS</h6>
                                ...
                                <div class="ai-input-area">
                                    <!-- Hidden File Inputs -->
                                    <input type="file" id="file_referensi_VMTS" name="file_referensi[]"
                                        accept=".docx,.pdf,.txt,.xlsx,.xls,.jpg,.jpeg,.png,.gif,.bmp,.webp" multiple
                                        style="display: none;">
                                    <input type="file" id="ocr_images_VMTS" name="ocr_images[]"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
                    ...
                    if (data.success) {
                        ...
                        if (successCount > 0 && textLength > 0) {
                            ocrMsg =
                                ...
                                `✓ Konten tervalidasi relevan dengan Laporan VMTS<br><br>` +
                            ...
                            appendAIMessage(`
                                <div style="background:#fef2f2;border-left:4px solid #dc2626;padding:16px;border-radius:8px;margin:8px 0;">
                                    <p style="color:#dc2626;margin:0 0 12px 0;font-weight:600;font-size:16px;">
                                        <i class="bi bi-exclamation-triangle-fill"></i> Gambar Tidak Relevan dengan Laporan VMTS
                                    </p>
            ...
            function appendAIMessageWithPreview(aiResponse, sections, cacheInfoHTML = '') {
                ...
                // Tampilkan tombol Generate Word jika response cukup panjang (lebih dari 500 karakter)
            ...
            btnCreateDraft.addEventListener('click', async function() {
                const periode = document.getElementById('periode_VMTS').value;
                const judul = document.getElementById('judul_laporan').value;

                if (!periode) {
                    alert('Pilih periode laporan terlebih dahulu.');
                    document.getElementById('periode_VMTS').focus();
                    return;
                }
                ...
                try {
                    ...
                    if (data.success && data.data && data.data.id) {
                        ...
                        // Disable periode, judul fields
                        document.getElementById('periode_VMTS').disabled = true;
            ...
            btnAskAI.addEventListener('click', async function() {
                ...
                try {
                    ...
                    if (periode) formData.append('periode_VMTS', periode);
                    ...
                    try {
                        response = await fetch(
                            '{{ route('gjm.buat-laporan.vmts.ai-prompt') }}', {
                                method: 'POST',
                                body: formData,
                                headers: {
                                    'Accept': 'application/json',
            ...
            async function saveAIPreviewToDatabase(laporanId, aiResponse, sections) {
                ...
                try {
                    ...
                    const response = await fetch('{{ route('gjm.buat-laporan.vmts.save-preview') }}', {
                        method: 'POST',
                        body: formData,
                        headers: {
                            'Accept': 'application/json',
            ...
            window.generateWordFromChat = async function() {
                ...
                try {
                    const response = await fetch('{{ route('gjm.buat-laporan.vmts.store') }}', {
                        method: 'POST',
                        body: formData,
                        headers: {

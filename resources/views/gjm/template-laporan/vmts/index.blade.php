...
@section('page-title', 'Kelola Template Laporan VMTS')
                ...
                <div class="alert-app-body">
                    ...
                    <div class="mt-2">
                        <button type="button" class="btn btn-sm btn-outline-danger"
                            ...
                            <i class="bi bi-trash"></i> Hapus Paksa ({{ session('warning')['usage_count'] }} laporan akan
                            kehilangan referensi template)
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h5 class="mb-1 d-flex align-items-center gap-2" style="font-weight: 600; color: #333;">
                        Kelola Template Laporan VMTS
                    </h5>
                    <p class="text-muted mb-0" style="font-size: 0.875rem;">Upload dan kelola template Word untuk Laporan
                        VMTS</p>
                </div>
                <div class="d-flex gap-2">
                    ...
                    <a href="{{ route('gjm.template-laporan.vmts.upload') }}" class="btn-reminder">
                ...
                <div class="d-flex align-items-start">
                    ...
                    <div>
                        ...
                        <p class="mb-2" style="font-size: 0.875rem; color: #495057; line-height: 1.6;">
                            ...
                            menghasilkan laporan VMTS yang konsisten.
                        </p>
                        <p class="mb-0" style="font-size: 0.8rem; color: #6c757d;">
                            ...
                            bullet points, dan format yang konsisten. AI akan meniru format ini dalam laporan yang
                            dihasilkan.
                        </p>
                    </div>
                </div>
                        ...
                        @forelse($templates as $template)
                            <tr>
                                ...
                                <td>
                                    <div>
                                        ...
                                        <span class="text-secondary"
                                            style="font-size: 0.85rem;">{{ $template->nama_file }}</span>
                                    </div>
                                ...
                                <td class="text-center">
                                    ...
                                    @else
                                        <span class="badge-gkm" style="background: #e9ecef; color: #6c757d;">Tidak
                                            Aktif</span>
                                    @endif
                                </td>
                                <td>
                                    ...
                                    <div class="d-flex flex-column align-items-center gap-2">
                                        <div class="btn-group" role="group">
                                            <a href="{{ route('gjm.template-laporan.download', $template->id) }}"
                                                class="btn btn-sm btn-outline-primary" title="Download Template">
                                                <i class="bi bi-download"></i>
                                            </a>

                                            <form
                                                action="{{ route('gjm.template-laporan.reindex', ['id' => $template->id, 'type' => 'vmts']) }}"
                                            ...
                                            <form
                                                action="{{ route('gjm.template-laporan.toggle', ['id' => $template->id, 'type' => 'vmts']) }}"
                                                method="POST" class="d-inline">
                                                @csrf
                                                <button type="submit"
                                                    ...
                                                    <i
                                                        class="bi bi-{{ $template->is_active ? 'toggle-on' : 'toggle-off' }}"></i>
                                                </button>
                                            </form>

                                            <form
                                                action="{{ route('gjm.template-laporan.destroy', ['id' => $template->id, 'type' => 'vmts']) }}"
                                                method="POST" class="d-inline"
                                        ...
                                        @if ($template->is_indexed)
                                            <small class="badge-gkm success" style="font-size: 0.7rem; color: #28a745;">
                                                <i class="bi bi-check-circle"></i> Indexed ({{ $template->total_chunks }}
                                                chunks)
                                            </small>
                                        @else
                            ...
                            <tr>
                                <td colspan="5" class="text-center py-5">
                                    <div class="empty-state">
                                        ...
                                        <p>Belum ada template. <a href="{{ route('gjm.template-laporan.vmts.upload') }}"
                                                style="color: #5B9BD5; font-weight: 600;">Upload template baru</a></p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
...
@push('scripts')
    <script>
        ...
        function forceDeleteTemplate(templateId, type) {
            if (confirm(
                    'PERHATIAN: Template akan dihapus dan semua laporan yang menggunakan template ini akan kehilangan referensi template. Lanjutkan?'
                    )) {

            ...
            <div class="col-xl-11 col-lg-12 mx-auto">
                ...
                <form id="laporanForm" enctype="multipart/form-data">
                    ...
                    <div class="monitoring-card mb-4" id="ai-prompt-section" style="overflow: hidden;">
                        ...
                        <div style="padding: 1.5rem;">
                            <!-- Laporan Kuesioner AI Assistant Interface -->
                            <style>
                            ...
                            <div class="ai-chat-wrapper">
                                <div class="ai-chat-header">
                                    ...
                                    <div>
                                        <h6>AI Assistant - Laporan Kuesioner</h6>
                                        <small>Siap membantu Anda membuat laporan kuesioner kepuasan mahasiswa</small>
                                    </div>
                                </div>
                                ...
                                <div class="ai-input-area">
                                    <!-- Hidden File Inputs -->
                                    <input type="file" id="file_referensi_kuesioner" name="file_referensi[]"
                                        accept=".docx,.pdf,.txt,.xlsx,.xls,.jpg,.jpeg,.png,.gif,.bmp,.webp" multiple
                                        style="display: none;">
                                    <input type="file" id="ocr_images_kuesioner" name="ocr_images[]"
                                    ...
                                    <div
                                        ...
                                        <textarea class="ai-textarea-inline" id="ai-prompt-input"
                                            placeholder="Deskripsikan laporan kuesioner yang ingin Anda buat..."></textarea>
...
@section('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const fileInput = document.getElementById('file_referensi_kuesioner');
            const ocrInput = document.getElementById('ocr_images_kuesioner');
            ...
            window.removeFile = function(index) {
                ...
                if (!promptInput.value.trim() && (selectedFiles.length + selectedOCRImages.length) > 0) {
                    ...
                    promptInput.placeholder = 'Deskripsikan laporan kuesioner yang ingin Anda buat...';
                }
            };
                    ...
                    if (data.success) {
                        ...
                        if (successCount > 0 && textLength > 0) {
                            ocrMsg =
                                ...
                                `✓ Konten tervalidasi relevan dengan Laporan Kuesioner<br><br>` +
                            ...
                            appendAIMessage(`
                                <div style="background:#fef2f2;border-left:4px solid #dc2626;padding:16px;border-radius:8px;margin:8px 0;">
                                    <p style="color:#dc2626;margin:0 0 12px 0;font-weight:600;font-size:16px;">
                                        <i class="bi bi-exclamation-triangle-fill"></i> Gambar Tidak Relevan dengan Laporan Kuesioner
                                    </p>
            ...
            btnAskAI.addEventListener('click', async function() {
                ...
                // VALIDATION 1: Files are now OPTIONAL (data can come from database)
                // Commented out to allow AI Assistant to work without file upload
                /*
                ...
                try {
                    ...
                    try {
                        response = await fetch(
                            '{{ route('gkm.laporan-kuesioner.ai-prompt') }}', {
                                method: 'POST',
                                body: formData,
                                headers: {
                                    'Accept': 'application/json',
                    ...
                    if (data.success) {
                        ...
                        if (data.auto_generated && data.download_url) {
                            console.log('🎉 Auto-generated laporan detected! Triggering auto-download...');
                            ...
                            // Trigger auto-download after 1.5 seconds
                            setTimeout(() => {
            ...
            async function saveAIPreviewToDatabase(laporanId, aiResponse, sections) {
                ...
                try {
                    ...
                    const response = await fetch('{{ route('gkm.laporan-kuesioner.save-preview') }}', {
                        method: 'POST',
                        body: formData,
                        headers: {
                            'Accept': 'application/json',
            ...
            window.generateWordFromChat = async function() {
                ...
                try {
                    const response = await fetch('{{ route('gkm.laporan-kuesioner.generate-word') }}', {
                        method: 'POST',
                        body: formData,
                        headers: {
                    ...
                    if (contentType && (contentType.includes('application/vnd.openxmlformats') ||
                        ...
                        let filename = 'Laporan_Kuesioner_' + Date.now() + '.docx';
            ...
            // ================================================================
            appendAIMessage(
                '<strong>Selamat datang di AI Assistant Laporan Kuesioner!</strong><br>Saya siap membantu Anda membuat laporan kuesioner kepuasan mahasiswa yang komprehensif. Silakan:<br><br>1. Klik <strong>"Buat Laporan Draft"</strong> terlebih dahulu<br>2. Ketik instruksi atau deskripsi laporan yang diinginkan<br>3. <em>(Opsional)</em> Upload file referensi jika Anda memiliki dokumen tambahan<br>4. Saya akan mengambil data kuesioner dari database dan membantu generate draft laporan untuk Anda'
            );

        });

        // ================================================================
        // Auto-Download Toast Notification Function
        // ================================================================
        function showAutoDownloadToast() {
            ...
            toast.innerHTML = `
                ...
                <div>
                    <div>File Word Berhasil Di-Generate!</div>
                    <div style="font-size: 12px; opacity: 0.9; font-weight: 400; margin-top: 4px;">
                        File sedang didownload ke perangkat Anda...
                    </div>
                </div>
            `;

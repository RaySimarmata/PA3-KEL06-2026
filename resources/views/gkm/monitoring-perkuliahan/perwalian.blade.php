...
@section('page-title', 'Kirim Reminder Perwalian')
    ...
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h5 class="mb-1">Kirim Reminder Perwalian & Persiapan Perkuliahan</h5>
                    <p class="text-muted mb-0">Pilih dosen wali yang akan dikirim reminder untuk perwalian dan persiapan perkuliahan</p>
                </div>
                <a href="{{ route('gkm.monitoring-perkuliahan.index') }}" class="btn btn-outline-secondary">
                    ...
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    @if($dosen->kelas_wali)
                                        <span class="badge bg-info">{{ $dosen->kelas_wali }}</span>
                                    @else
                                        <span class="text-muted">-</span>
                                    @endif
                                </td>
                                <td>
                                    <span class="badge bg-success">Dosen Wali</span>
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="6" class="text-center py-4">Tidak ada data dosen wali</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
...
function generateMessage() {
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            alert('Gagal generate pesan: ' + (data.message || 'Unknown error'));
    ...
    .catch(error => {
        ...
        alert('Terjadi kesalahan saat generate pesan: ' + error.message);
        messageTextarea.value = '';
        messageTextarea.disabled = false;
    });
}

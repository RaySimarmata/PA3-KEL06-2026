/**
 * AI Prompt Assistant untuk Laporan vmts
 * Menyediakan fitur AI assistant dengan conversation history untuk membantu pengguna dalam membuat laporan vmts
 * Version: 1.1.0 - Added file upload validation
 * Last Updated: 2026-05-27
 */

class AIPromptAssistantVMTS {
    constructor() {
        ...
        console.log('AI Prompt Assistant vmts v1.1.0 - File Upload Validation Enabled');
    }
    ...
    init() {
        ...
        console.log('AI Prompt Assistant vmts initialized');
    }

    createAssistantUI() {
        // Cek apakah UI sudah ada
        if (document.getElementById('ai-assistant-vmts')) return;

        const assistantHTML = `
            <div id="ai-assistant-vmts" class="ai-assistant-container" style="display: none;">
                <div class="ai-assistant-header">
                    <h4><i class="fas fa-robot"></i> AI Assistant - Laporan vmts</h4>
                    <div class="ai-assistant-controls">
                        <button type="button" class="btn btn-sm btn-outline-secondary" onclick="aiAssistantvmts.clearConversation()" title="Clear Conversation">
                            <i class="fas fa-trash"></i>
                        </button>
                        <button type="button" class="btn btn-sm btn-outline-info" onclick="aiAssistantvmts.showSuggestions()" title="Show Suggestions">
                            <i class="fas fa-lightbulb"></i>
                        </button>
                        <button type="button" class="btn-close-assistant" onclick="aiAssistantvmts.toggleAssistant()">
                            <i class="fas fa-times"></i>
                        </button>
                    </div>
                </div>
                <div class="ai-assistant-body">
                    <div class="ai-conversation" id="ai-conversation-vmts">
                        <div class="ai-message ai-message-system">
                            <i class="fas fa-robot"></i>
                            <div class="message-content">
                                <p>Halo! Saya AI Assistant untuk membantu Anda membuat laporan vmts. Saya dapat membantu dengan:</p>
                                <ul>
                                    <li>Memberikan saran konten laporan vmts</li>
                    ...
                    <div class="ai-input-container">
                        <div class="file-upload-area" id="file-upload-area-vmts">
                            <input type="file" id="file-input-vmts" multiple 
                                   accept=".pdf,.doc,.docx,.txt,.xlsx,.xls,.jpg,.jpeg,.png,.gif,.webp"
                                   style="display: none;">
                            <div class="upload-zone" onclick="document.getElementById('file-input-vmts').click()">
                                <i class="fas fa-cloud-upload-alt"></i>
                                <span>Upload dokumen pendukung (opsional)</span>
                            </div>
                            <div class="uploaded-files" id="uploaded-files-vmts"></div>
                        </div>
                        <div class="input-group">
                            <input type="text" id="ai-input-vmts" class="form-control" 
                                   placeholder="Tanyakan sesuatu tentang laporan vmts..." 
                                   onkeypress="if(event.key==='Enter') aiAssistantvmts.sendMessage()">
                            <button class="btn btn-primary" onclick="aiAssistantvmts.sendMessage()" id="ai-send-btn-vmts">
    ...
    bindEvents() {
        ...
        const input = document.getElementById('ai-input-vmts');
        if (input) {
    ...
    bindFileUploadEvents() {
        const fileInput = document.getElementById('file-input-vmts');
        if (fileInput) {
        ...
        // Drag and drop
        const uploadZone = document.querySelector('#file-upload-area-vmts .upload-zone');
        if (uploadZone) {
    ...
    async handleFileUpload(files) {
        const uploadedFilesContainer = document.getElementById('uploaded-files-vmts');
        ...
        fileArray.forEach((file) => {
            ...
            if (file.type.startsWith('image/') || ['jpg', 'jpeg', 'png', 'gif', 'webp'].includes(extension)) {
                ...
                this.addMessage('system', `⚠️ File ${file.name} memiliki format yang tidak didukung`);
            }
        });

        // Handle Excel files - PRIORITAS TERTINGGI
        if (excelFiles.length > 0) {
            console.log('📊 Excel files detected:', excelFiles.length);

            excelFiles.forEach(file => {
                this.displayUploadedFile(file, uploadedFilesContainer);
            });

            this.addMessage('system', `✅ <strong>${excelFiles.length} file Excel berhasil diupload!</strong><br><br>📊 File Excel akan dianalisis oleh AI untuk mengekstrak data survei VMTS.<br><br><strong>💡 Langkah selanjutnya:</strong><br>Ketik instruksi seperti:<br>• "Analisis data Excel dan buat laporan lengkap"<br>• "Buat laporan VMTS berdasarkan data Excel"<br>• "Ekstrak dan analisis semua data dari Excel"`);

            // Focus on input with specific placeholder
            setTimeout(() => {
                const input = document.getElementById('ai-input-vmts');
                if (input) {
                    input.focus();
                    input.placeholder = '📊 Ketik instruksi untuk menganalisis data Excel...';
                }
            }, 500);
        }

        // Handle other document files (PDF, Word, etc.)
        if (otherFiles.length > 0) {
            console.log('📄 Document files detected:', otherFiles.length);

            otherFiles.forEach(file => {
                this.displayUploadedFile(file, uploadedFilesContainer);
            });

            this.addMessage('system', `✅ ${otherFiles.length} dokumen berhasil diupload.`);
        }

        // Handle image files - validate with OCR
        if (imageFiles.length > 0) {
            console.log('🖼️ Image files detected:', imageFiles.length);
            ...
            try {
                ...
                formData.append('report_type', 'laporan_vmts');
                ...
                this.addMessage('system', `⚠️ Validasi gambar gagal: ${error.message}<br><br>Gambar tetap diupload, tetapi mungkin tidak relevan dengan Laporan vmts.`);
            }
        }
    }
    ...
    getLaporanId() {
        ...
        if (!laporanId) {
            const form = document.querySelector('form[action*="laporan-vmts"]');
            if (form) {
    ...
    async callAIService(message, files = []) {
        ...
        if (context.periode_vmts) {
            formData.append('periode_vmts', context.periode_vmts);
        }
        ...
        const response = await fetch('/gjm/laporan-vmts/ai-prompt', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '',
        ...
        if (modifySectionMatch) {
            ...
            if (lastFullDraft) {
                ...
                this.addMessage('system', '⚠️ <strong>Peringatan:</strong> Tidak ada draft lengkap sebelumnya. Untuk hasil terbaik, buat draft lengkap terlebih dahulu dengan perintah "Buat laporan vmts lengkap", lalu ubah bagian yang diinginkan.');
            }
        }
    ...
    getPageContext() {
        ...
        // Ambil informasi periode vmts jika ada
        const periodeSelect = document.querySelector('select[name*="periode_vmts"]');
        if (periodeSelect && periodeSelect.value) {
            context.periode_vmts = periodeSelect.value;
        }
    ...
    addMessage(type, content) {
        const conversation = document.getElementById('ai-conversation-vmts');
    ...
    updateSendButton(isLoading) {
        const btn = document.getElementById('ai-send-btn-vmts');
        const input = document.getElementById('ai-input-vmts');
    ...
    getSuggestions() {
        return [
            "Buatkan draft laporan vmts lengkap dengan semua bagian",
            "Bagaimana struktur laporan vmts yang baik?",
            "Apa saja komponen yang harus ada dalam laporan vmts?",
            "Bagaimana cara menganalisis data perkuliahan vmts?",
            "Berikan template laporan vmts yang sesuai standar",
    ...
    showSuggestions() {
        ...
        const suggestionsHTML = randomSuggestions.map(suggestion =>
            `<button class="btn btn-outline-primary btn-sm suggestion-btn mb-1 me-1" onclick="aiAssistantvmts.useSuggestion('${suggestion.replace(/'/g, '\\\'')}')">${suggestion}</button>`
        ).join('');
    ...
    useSuggestion(suggestion) {
        document.getElementById('ai-input-vmts').value = suggestion;
        this.sendMessage();
    }
        ...
        conversation.innerHTML = `
            <div class="ai-message ai-message-system">
                ...
                <div class="message-content">
                    ...
                    <p>Anda bisa upload dokumen pendukung dan saya akan membantu menganalisisnya untuk laporan vmts.</p>
                </div>
            </div>
        `;
        this.currentConversation = [];
...
document.addEventListener('DOMContentLoaded', function () {
    aiAssistantvmts = new AIPromptAssistantVMTS();
});

// Export untuk penggunaan di tempat lain jika diperlukan
if (typeof module !== 'undefined' && module.exports) {
    module.exports = AIPromptAssistantVMTS;
}
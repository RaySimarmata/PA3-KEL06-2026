/**
 * AI Prompt Assistant untuk Laporan Triwulan
 * Menyediakan fitur AI assistant dengan conversation history untuk membantu pengguna dalam membuat laporan triwulan
 * Version: 1.1.0 - Added file upload validation
 * Last Updated: 2026-05-27
 */

class AIPromptAssistantTriwulan {
    constructor() {
        ...
        console.log('AI Prompt Assistant Triwulan v1.1.0 - File Upload Validation Enabled');
    }
    ...
    init() {
        ...
        console.log('AI Prompt Assistant Triwulan initialized');
    }

    createAssistantUI() {
        // Cek apakah UI sudah ada
        if (document.getElementById('ai-assistant-triwulan')) return;

        const assistantHTML = `
            <div id="ai-assistant-triwulan" class="ai-assistant-container" style="display: none;">
                <div class="ai-assistant-header">
                    <h4><i class="fas fa-robot"></i> AI Assistant - Laporan Triwulan</h4>
                    <div class="ai-assistant-controls">
                        <button type="button" class="btn btn-sm btn-outline-secondary" onclick="aiAssistantTriwulan.clearConversation()" title="Clear Conversation">
                            <i class="fas fa-trash"></i>
                        </button>
                        <button type="button" class="btn btn-sm btn-outline-info" onclick="aiAssistantTriwulan.showSuggestions()" title="Show Suggestions">
                            <i class="fas fa-lightbulb"></i>
                        </button>
                        <button type="button" class="btn-close-assistant" onclick="aiAssistantTriwulan.toggleAssistant()">
                            <i class="fas fa-times"></i>
                        </button>
                    </div>
                </div>
                <div class="ai-assistant-body">
                    <div class="ai-conversation" id="ai-conversation-triwulan">
                    ...
                    <div class="ai-input-container">
                        <div class="file-upload-area" id="file-upload-area-triwulan">
                            <input type="file" id="file-input-triwulan" multiple 
                                   accept=".pdf,.doc,.docx,.txt,.xlsx,.xls,.jpg,.jpeg,.png,.gif,.webp"
                                   style="display: none;">
                            <div class="upload-zone" onclick="document.getElementById('file-input-triwulan').click()">
                                <i class="fas fa-cloud-upload-alt"></i>
                                <span>Upload dokumen pendukung (opsional)</span>
                            </div>
                            <div class="uploaded-files" id="uploaded-files-triwulan"></div>
                        </div>
                        <div class="input-group">
                            <input type="text" id="ai-input-triwulan" class="form-control" 
                                   placeholder="Tanyakan sesuatu tentang laporan triwulan..." 
                                   onkeypress="if(event.key==='Enter') aiAssistantTriwulan.sendMessage()">
                            <button class="btn btn-primary" onclick="aiAssistantTriwulan.sendMessage()" id="ai-send-btn-triwulan">
        ...
        const floatingBtn = `
            <button id="ai-floating-btn-triwulan" class="ai-floating-btn" onclick="aiAssistantTriwulan.toggleAssistant()" title="AI Assistant Triwulan">
                <i class="fas fa-robot"></i>
                <span class="floating-btn-label">AI Triwulan</span>
            </button>
        `;
    ...
    bindEvents() {
        ...
        const input = document.getElementById('ai-input-triwulan');
        if (input) {
    ...
    bindFileUploadEvents() {
        const fileInput = document.getElementById('file-input-triwulan');
        if (fileInput) {
        ...
        // Drag and drop
        const uploadZone = document.querySelector('#file-upload-area-triwulan .upload-zone');
        if (uploadZone) {
    ...
    async handleFileUpload(files) {
        const uploadedFilesContainer = document.getElementById('uploaded-files-triwulan');
        ...
        fileArray.forEach((file, index) => {
            ...
            if (file.size > 10 * 1024 * 1024) {
                this.addMessage('system', `âŒ File ${file.name} terlalu besar (max 10MB)`);
                return;
            }
        ...
        if (imageFiles.length > 0) {
            try {
                ...
                if (data.success) {
                    ...
                    let successMsg = `âœ… ${data.stats.accepted} gambar berhasil divalidasi dan diupload.`;

                    // Show warning if some images were rejected
                    if (data.rejected_images && data.rejected_images.length > 0) {
                        successMsg += `<br><br>âš ï¸ ${data.rejected_images.length} gambar ditolak:<br>`;
                        data.rejected_images.forEach(rejected => {
                            successMsg += `<br>â€¢ <strong>${rejected.filename}</strong><br>`;
                            successMsg += `  Alasan: ${rejected.reason}<br>`;
                            successMsg += `  Confidence: ${rejected.confidence}`;
                        });
                    }

                    successMsg += `<br><br><strong>ðŸ’¡ Jangan lupa:</strong> Berikan instruksi di kolom chat untuk memproses gambar yang diupload.`;
                    ...
                    setTimeout(() => {
                        ...
                        if (input) {
                            input.focus();
                            input.placeholder = 'âš ï¸ Ketik instruksi untuk memproses gambar yang diupload...';
                        }
                    }, 500);

                } else {
                    // All images rejected or error
                    let errorMsg = `âŒ ${data.message || 'Validasi gambar gagal'}`;

                    if (data.rejected_images && data.rejected_images.length > 0) {
                        errorMsg += '<br><br>ðŸ“‹ Detail Penolakan:<br>';
                        data.rejected_images.forEach(rejected => {
                            errorMsg += `<br>â€¢ <strong>${rejected.filename}</strong><br>`;
                ...
                this.addMessage('system', `âš ï¸ Validasi gambar gagal: ${error.message}<br><br>Gambar tetap diupload, tetapi mungkin tidak relevan dengan Laporan Triwulan.`);
            }
        }
        ...
        if (nonImageCount > 0) {
            this.addMessage('system', `âœ… ${nonImageCount} dokumen non-gambar berhasil diupload.`);
        }
    }
    ...
    getLaporanId() {
        ...
        if (!laporanId) {
            const form = document.querySelector('form[action*="laporan-triwulan"]');
            if (form) {
    ...
    toggleAssistant() {
        const assistant = document.getElementById('ai-assistant-triwulan');
        const floatingBtn = document.getElementById('ai-floating-btn-triwulan');
        ...
        if (assistant.style.display === 'none' || assistant.style.display === '') {
            ...
            // Focus pada input
            setTimeout(() => {
                document.getElementById('ai-input-triwulan')?.focus();
            }, 100);
        } else {
    ...
    async sendMessage() {
        ...
        if (!message || message.length === 0) {
            console.warn('âŒ VALIDATION FAILED: No message provided');
            if (hasFiles) {
                this.addMessage('system', '<strong>âš ï¸ INSTRUKSI WAJIB DIISI!</strong><br><br>Anda telah mengupload <strong>' + uploadedFiles.length + ' file</strong>, tetapi belum memberikan instruksi.<br><br><strong>Silakan ketik instruksi Anda terlebih dahulu</strong>, misalnya:<br>â€¢ "Analisis dokumen ini dan buat ringkasan"<br>â€¢ "Buat laporan berdasarkan data yang diupload"<br>â€¢ "Ekstrak informasi penting dari dokumen"<br><br><em>File tidak akan diproses tanpa instruksi yang jelas.</em>');
        ...
        if (message.length < 5) {
            console.warn('âŒ VALIDATION FAILED: Message too short (' + message.length + ' chars)');
            this.addMessage('system', '<strong>âš ï¸ Instruksi terlalu singkat!</strong><br><br>Instruksi Anda hanya <strong>' + message.length + ' karakter</strong>.<br>Silakan berikan instruksi yang lebih jelas dan spesifik (minimal 5 karakter).<br><br>Contoh instruksi yang baik:<br>â€¢ "Analisis dokumen ini"<br>â€¢ "Buat ringkasan"<br>â€¢ "Ekstrak data penting"');
        ...
        console.log('âœ… VALIDATION PASSED: Proceeding with request');
    ...
    async callAIService(message, files = []) {
        ...
        if (context.periode_triwulan) {
            formData.append('periode_triwulan', context.periode_triwulan);
        }
        ...
        const response = await fetch('/gjm/laporan-triwulan/ai-prompt', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '',
        ...
        if (modifySectionMatch) {
            ...
            if (lastFullDraft) {
                ...
                console.warn('âš ï¸ User asked to modify section but no full draft exists!');
                console.warn('âš ï¸ Please create a full draft first with "Buat laporan lengkap"');

                // Show warning to user
                this.addMessage('system', 'âš ï¸ <strong>Peringatan:</strong> Tidak ada draft lengkap sebelumnya. Untuk hasil terbaik, buat draft lengkap terlebih dahulu dengan perintah "Buat laporan triwulan lengkap", lalu ubah bagian yang diinginkan.');
            }
        }
    ...
    getPageContext() {
        ...
        // Ambil informasi periode triwulan jika ada
        const periodeSelect = document.querySelector('select[name*="periode_triwulan"]');
        if (periodeSelect && periodeSelect.value) {
            context.periode_triwulan = periodeSelect.value;
        }
    ...
    addMessage(type, content) {
        const conversation = document.getElementById('ai-conversation-triwulan');
    ...
    updateSendButton(isLoading) {
        const btn = document.getElementById('ai-send-btn-triwulan');
        const input = document.getElementById('ai-input-triwulan');
    ...
    getSuggestions() {
        return [
            "Buatkan draft laporan triwulan lengkap dengan semua bagian",
            "Bagaimana struktur laporan triwulan yang baik?",
            "Apa saja komponen yang harus ada dalam laporan triwulan?",
            "Bagaimana cara menganalisis data perkuliahan triwulan?",
            "Berikan template laporan triwulan yang sesuai standar",
    ...
    showSuggestions() {
        ...
        const suggestionsHTML = randomSuggestions.map(suggestion =>
            `<button class="btn btn-outline-primary btn-sm suggestion-btn mb-1 me-1" onclick="aiAssistantTriwulan.useSuggestion('${suggestion.replace(/'/g, '\\\'')}')">${suggestion}</button>`
        ).join('');
    ...
    useSuggestion(suggestion) {
        document.getElementById('ai-input-triwulan').value = suggestion;
        this.sendMessage();
    }
        ...
        conversation.innerHTML = `
            <div class="ai-message ai-message-system">
                ...
                <div class="message-content">
                    ...
                    <p>Anda bisa upload dokumen pendukung dan saya akan membantu menganalisisnya untuk laporan triwulan.</p>
                </div>
            </div>
        `;
        this.currentConversation = [];
...
// Inisialisasi global
let aiAssistantTriwulan;

// Inisialisasi ketika DOM ready
document.addEventListener('DOMContentLoaded', function () {
    aiAssistantTriwulan = new AIPromptAssistantTriwulan();
});

// Export untuk penggunaan di tempat lain jika diperlukan
if (typeof module !== 'undefined' && module.exports) {
    module.exports = AIPromptAssistantTriwulan;
}

...
/**
 * AI Prompt Service khusus Laporan Triwulan
 *
...
class PromptTriwulanService
{
    ...
    // Simpan extracted text terakhir agar bisa dipakai di follow-up
     ...
     * dan hasilkan draft konten laporan Triwulan.
     *
     * Mendukung:
     * - Prompt pertama (dengan atau tanpa file)
     * - Follow-up dari user setelah melihat draft (AI ikut instruksi baru)
     * - previousDraft bisa dipass dari frontend untuk multi-turn
     */
    public function processPrompt(
        ...
        Log::info('=== PromptTriwulanService::processPrompt START ===', [
        ...
        try {
            ...
            if ($referenceFile) {
                ...
                $this->lastExtractedText = $fileContext;
            }
            ...
            if (!empty($previousDraft)) {
                // User memberikan instruksi lanjutan — multi-turn
                ...
                // Pesan pertama — bangun user message lengkap
            ...
            Log::error('PromptTriwulanService::processPrompt FAILED', [
                'error' => $e->getMessage(),
     ...
     * User cukup upload file, AI langsung membaca dan merangkum isinya,
     * hasilnya muncul di Preview Draft Laporan.
     */
    public function readAndSummarizeFile(
        ...
        Log::info('=== PromptTriwulanService::readAndSummarizeFile START ===', [
        ...
        try {
            ...
            $periode = $periodeInfo['label'] ?? 'Triwulan';
            $tahun   = $periodeInfo['tahun'] ?? date('Y');
            $judul   = $periodeInfo['judul'] ?? 'Laporan Triwulan GJM';

            // 3. System prompt untuk pembacaan file
            $systemPrompt  = "Anda adalah AI assistant yang ahli membaca, menganalisis, dan merangkum dokumen akademik.\n";
            $systemPrompt .= "Anda membantu membuat Laporan Triwulan GJM (Gugus Jaminan Mutu) Fakultas Vokasi Institut Teknologi Del.\n";
            $systemPrompt .= "Anda akan diberikan isi dokumen laporan GJM/GKM dan diminta membacanya secara menyeluruh,\n";
            $systemPrompt .= "kemudian membuat rangkuman dan draft laporan Triwulan berdasarkan isi dokumen tersebut.\n";
            ...
            $systemPrompt .= "\nInformasi Laporan:\n";
            $systemPrompt .= "- Jenis     : Laporan Triwulan GJM\n";
            ...
            if (!empty($templateContext)) {
                $systemPrompt .= "\nFormat template laporan yang harus diikuti:\n" . $templateContext;
            }

            // 4. Instruksi tambahan untuk struktur output
            ...
            // 5. Panggil Claude untuk membaca & merangkum
            ...
            Log::error('PromptTriwulanService::readAndSummarizeFile FAILED', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ]);
    ...
    private function buildSystemPrompt(array $periodeInfo, string $templateContext): string
    {
        ...
        $system .= "Laporan Triwulan GJM (Gugus Jaminan Mutu) Fakultas Vokasi Institut Teknologi Del.\n";
        ...
        $system .= "- Jenis     : Laporan Triwulan GJM\n";
    ...
    private function buildUserMessage(string $userPrompt, string $fileContext, array $periodeInfo): string
    {
        ...
        $message .= "Hasilkan draft laporan triwulan lengkap dengan struktur bagian-bagian (gunakan heading # Markdown).\n";
    ...
    private function extractReferenceFile(UploadedFile $file): string
    {
        try {
            ...
            Log::info('Extracting reference file (triwulan)', [
            ...
            if (!$extracted['success'] || empty($extracted['text'])) {
                Log::warning('Reference file extraction empty (triwulan)');
                return '';
            }
            ...
            Log::warning('Failed to extract reference file (triwulan)', ['error' => $e->getMessage()]);
            return '';
        }
    }
    ...
    private function fetchTemplateContext(int $templateId): string
    {
        try {
            ...
            $retrieval = $this->ragRetrieval->retrieveContext(
                'Struktur format dan isi Laporan Triwulan GJM',
                ['template_id' => $templateId]
            );

            return $retrieval['context_text'] ?? '';
        } catch (\Exception $e) {
            Log::warning('Failed to fetch template context (triwulan)', ['error' => $e->getMessage()]);
            return '';
        }
    }

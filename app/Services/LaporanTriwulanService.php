...
class LaporanTriwulanService
{
    ...
    {
        Log::info("=== Starting Laporan Triwulan Generation ===", ['laporan_id' => $laporanId]);
        ...
        try {
            ...
            $userMessage = $instruksi['user_message'] ?? 'Buat laporan triwulan';
            ...
            Log::info('RAGAS evaluation completed for Triwulan', [
                'laporan_id' => $laporanId,
    ...
    private function extractPlaceholdersFromAIPreview($preview, $periode, $tahun, $laporan)
    {
        ...
        if ($hasOCRData) {
            ...
            Log::info("Laporan generated with OCR data integration", [
                ...
                'ocr_text_length' => strlen((string)$combinedText),
     ...
     * Preserve structure but remove markdown syntax and escape curly braces
     */
    ...
    /**
     * Generate Laporan Triwulan (FALLBACK - jika tidak ada AI draft)
     */
    public function generateLegacy($laporanId)
    {
        ...
        try {
            ...
            $instruksi = json_decode($laporan->instruksi_prompt, true);
            $periode = $instruksi['periode'] ?? 'Triwulan';  // "Triwulan Ganjil" atau "Triwulan Genap"
            $periodeTriwulan = $instruksi['periode_Triwulan'] ?? 'ganjil';  // "ganjil" atau "genap"
            $tahun = $instruksi['tahun'] ?? date('Y');
            $judul = $instruksi['judul'] ?? 'Laporan Triwulan';
            ...
            $result = $this->aiAgentService->generateDocument($prompt, [
                'format' => 'placeholders',
                'template_id' => $laporan->template_id,
                'document_type' => 'laporan_triwulan',
                'placeholders' => [
                    'PERIODE' => $periode,
    ...
    /**
     * Build prompt for Triwulan report
     */
    private function buildPrompt($dataContext, $templateContext, $periode, $tahun, $judul)
    {
        $prompt = "Anda adalah AI Agent yang bertugas membuat Laporan Triwulan untuk Gugus Jaminan Mutu (GJM) Fakultas Vokasi Institut Teknologi Del.\n\n";
    ...
    private function preparePlaceholders($result, $periode, $tahun, $laporan)
    {
        ...
        Log::info('TAHUN_AKADEMIK calculated for Triwulan', [
     ...
     * e.g. "{Some text returned by AI}" â†’ "Some text returned by AI"
     */
    private function cleanAiValue(string $value): string
    {
        ...
        if (str_starts_with($value, '{') && str_ends_with($value, '}')) {
            ...
            // Only strip if inner doesn't itself contain unbalanced braces
            if (substr_count($inner, '{') === substr_count($inner, '}')) {
                $value = trim($inner);
            }
        }

        // Escape any remaining curly braces to prevent template parsing errors
        // This prevents issues like "Unclosed '{' on line X"
    ...
    private function generateWordDocument($laporan, $placeholders)
    {
        ...
        $fileName = 'laporan_triwulan_' . time() . '.docx';
    ...
    {
        $fixedPath = sys_get_temp_dir() . '/Triwulan_fixed_' . time() . '.docx';
    ...
    private function mergeRunsInParagraphs(string $xml): string
    {
        ...
        return preg_replace_callback(
            ...
            // {{VAR}} â†’ {VAR}, {{ VAR}} â†’ {VAR}, { VAR} â†’ {VAR}, {VAR } â†’ {VAR}
    ...
    private function extractContextsFromPreview($preview)
    {
        ...
        try {
            ...
            if (empty($contexts)) {
                $contexts = ['AI preview laporan triwulan'];
            }
            ...
            return ['AI preview laporan triwulan'];
        }
    }

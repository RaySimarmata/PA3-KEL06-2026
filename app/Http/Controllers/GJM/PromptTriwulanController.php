...
/**
 * PromptTriwulanController
 *
 * Menangani AI Prompt Assistant untuk Laporan triwulan.
...
class PromptTriwulanController extends Controller
{
    ...
    /**
     * Endpoint: POST /buat-laporan/triwulan/read-file
     *
     ...
     *
     * CATATAN: Endpoint ini dipanggil hanya ketika user mengklik tombol â†‘ (Generate),
    ...
    public function readFile(Request $request)
    {
        ...
        try {
            ...
            Log::info('PromptTriwulan: file received', ['name' => $fileName, 'path' => $fullPath]);
            ...
            $periode          = $request->input('periode_triwulan', '');
            ...
            Log::error('PromptTriwulan readFile error', ['error' => $e->getMessage()]);
            return response()->json([
                'success' => false,
    ...
    /**
     * Endpoint: POST /buat-laporan/triwulan/prompt
     *
    ...
    /**
     * Build system prompt kontekstual untuk laporan triwulan.
     */
    private function buildSystemPrompt(string $type, string $periode = '', string $judul = ''): string
    {
        ...
        // Determine triwulan label
        $triwulanLabel = '';
        if (stripos($periode, 'ganjil') !== false) {
            $triwulanLabel = 'triwulan Ganjil';
        } elseif (stripos($periode, 'genap') !== false) {
            $triwulanLabel = 'triwulan Genap';
        } else {
            $triwulanLabel = $periode;
        }
...
4. Gunakan heading Markdown dengan format yang TEPAT:
   - # untuk judul utama (Laporan triwulan Ganjil)
...
REMINDER: Pastikan output Anda memiliki SEMUA 7 bagian utama dengan sub-bagian yang lengkap. Jangan pernah skip bagian apapun.";
    ...
    {
        try {
            // Determine date range based on triwulan period
            $currentYear = date('Y');
            $startDate = null;
            $endDate = null;

            if ($periode === 'ganjil') {
                // triwulan Ganjil: August - January
                $startDate = Carbon::create($currentYear, 8, 1);
                $endDate = Carbon::create($currentYear + 1, 1, 31);
            } elseif ($periode === 'genap') {
                // triwulan Genap: February - July
    ...
    {
        return 'âš  AI tidak memberikan respons. Periksa konfigurasi API key di file .env:<br><br>' .
